...
const TimelineSegments = ({data = [], totalTrackLength = 0}) => {
  ...
  useEffect(() => {
    ...
    setSegments(generateSegment(lines))
  }, [data])

  const generateSegment = (lines) => {
    let segments = []
    for (let i = 0; i < lines.length; i++) {
      for (let j = 0; j < lines[i].length; j++) {
        const segment = lines[i][j]
        segment && segments.push(<Segment key={segment.id} segment={segment} line={i} totalTrackLength={totalTrackLength}/>)
      }
    }
    return segments
  }
